...
func loadGatewayConfig() (gatewayConfig, error) {
	...
	if environment != "development" && environment != "test" && environment != "production" {
		return gatewayConfig{}, fmt.Errorf("TOOLPLANE_ENV_MODE must be one of development, test, or production")
	}

	allowInsecureBackend := boolEnv("TOOLPLANE_MCP_ALLOW_INSECURE_BACKEND", environment != "production")
	if environment == "production" && allowInsecureBackend {
		return gatewayConfig{}, fmt.Errorf("TOOLPLANE_MCP_ALLOW_INSECURE_BACKEND is not allowed when TOOLPLANE_ENV_MODE=production")
	}

	rawOrigins := strings.TrimSpace(os.Getenv("TOOLPLANE_MCP_ALLOWED_ORIGINS"))
	cfg := gatewayConfig{
	...
	if rawOrigins == "" {
		...
		return gatewayConfig{}, fmt.Errorf("TOOLPLANE_MCP_ALLOWED_ORIGINS is required outside development or test mode")
	}
	...
	for _, origin := range strings.Split(rawOrigins, ",") {
		...
		if trimmed == "*" {
			if environment == "production" {
				return gatewayConfig{}, fmt.Errorf("TOOLPLANE_MCP_ALLOWED_ORIGINS=* is not allowed in production")
			}
			cfg.allowAnyOrigin = true
	...
	if len(cfg.allowedOrigins) == 0 && !cfg.allowAnyOrigin {
		return gatewayConfig{}, fmt.Errorf("TOOLPLANE_MCP_ALLOWED_ORIGINS must contain at least one origin")
	}

	return cfg, nil
}

func (cfg gatewayConfig) matchOrigin(origin string) (string, bool) {
...
func (cfg gatewayConfig) corsSummary() string {
	if cfg.allowAnyOrigin {
...
func (cfg gatewayConfig) backendSecuritySummary() string {
