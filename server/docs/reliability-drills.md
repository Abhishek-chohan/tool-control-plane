...
- No exactly-once guarantee for non-idempotent tools: fencing stops stale writers after a reclaim, and `idempotency_key` deduplicates create retries, but a reclaimed request still re-executes under a fresh lease epoch — the key does not prevent re-execution of the same request.
