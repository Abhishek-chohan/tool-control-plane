...
- No exactly-once guarantee for non-idempotent tools: fencing stops stale writers after a reclaim, but the contract still has no client-supplied idempotency key, so a reclaimed request re-executes under a fresh lease epoch.
